...
- **Key-map entry = 3 bytes** at `13 + keyIndex*3` (`readKeyMap`/`writeKeyMap` in `hid/config.ts`, mirroring `Flydigi.ControllerSdk.cs:4493-4573`): `b0==32` → 宏(Macro); `b2>0` → 连发(Continuous) `[target, enableType, freq]`; `b0==0xFE`/`>31` → 键盘/多功能(serialised marker only). **单键 + 连发 are fully read/write; 宏 & 键盘/多功能 are detected and preserved byte-for-byte but read-only** (their payloads live in a separate protobuf region ~offset 230, not yet reversed).
